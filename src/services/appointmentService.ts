...
export class AppointmentService {
   ...
   */
  static async getTimeSlots(startDate?: string, endDate?: string): Promise<DaySchedule[]> {
    const params = new URLSearchParams();
    if (startDate) params.append('startDate', startDate);
    if (endDate) params.append('endDate', endDate);
    
    const queryString = params.toString();
    const endpoint = `/appointments/slots${queryString ? `?${queryString}` : ''}`;
   ...
   * Book a time slot
   */
  static async bookTimeSlot(slotId: string, bookingData: { patientName: string; description?: string }): Promise<TimeSlot> {
